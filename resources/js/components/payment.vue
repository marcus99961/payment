            ...
            <div class="card">
                <div class="card-header bg-primary text-light">
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <button @click="createPayment"  class="btn-info btn-sm float-end">New Payment</button>
                            <!-- <button @click="bulkDelete"  class="ml btn-danger btn-sm">Test Selected</button> -->
                        </div>
                    </div>
                    ...
                    <div class="shadow p-3 mb-5 bg-body rounded">
                        ...
                        <table class="table">
                            ...
                            <tr v-for="(payment, index) in payments " :key="index" class="bg-transparent">
                                ...
                                <td><div v-if="payment.description.length<19">{{payment.description}}</div>
                                    <div class="myDIV" v-else >{{ payment.description.substring(0,19)+".." }}</div>
                                    <span class="hide">{{ payment.description }}</span>
                                </td>
                                <td class="digit">{{payment.currency}} {{payment.amount}}</td>


                                <td>
                                    <button @click="editPayment(payment)" v-if="payment.user_id==current_user.id" class="btn btn-primary btn-sm mx-1">Edit</button>

                                    <button @click="removePayment(payment)" v-if="payment.user_id==current_user.id" class="btn btn-danger btn-sm mx-1">Delete</button>

                                     <button @click="form(payment)" class="btn btn-primary btn-sm mx-1">Form</button>
                                    <button @click="paid(payment)" v-if="current_user.role_id==2" class="btn btn-info btn-sm mx-1">Paid</button>
        ...
        <div :class="`modal-dialog ${!deleteMode ? 'modal-lg': 'modal-sm'}`">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title bg-primary" id="paymentModalLabel" v-show="!deleteMode && !paidMode"> {{!editMode ? 'Add New Payment': 'Update Payment' }} </h5>
                    ...
                    <div class="row mt-3" v-show="!deleteMode && !paidMode">
                        <div class="col-md-8">
                            <div class="form-group">
                                ...
                                <span class="text-danger" v-if="errors.ac_name">{{ errors.ac_name[0] }}</span>
                            </div>
                        </div>
...
export default {
    ...
    methods: {
        ...
        selectedFile(e){
            ...
            let reader = new FileReader();
            reader.onloadend= () => {
                this.paymentData.attached_file = reader.result;
            }
            reader.readAsDataURL(file);
        },
        storePayment(){
